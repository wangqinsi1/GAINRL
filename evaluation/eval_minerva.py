...
def evaluate_model(
    ...
    for start in range(0, total_samples, batch_size):
        ...
        for j, out in enumerate(outputs):
            ...
            pred = extract_xml_answer(response).replace(" ", "").replace("\n", "")
            truth = (
                re.sub(r'\$(.*?)\$', r'\1', batch["answer"][j][0])
                .replace(" ", "")
                .replace("\n", "")
            )
    ...
    if save_results:
        fname = f"minerva_eval_{datetime.now():%Y%m%d_%H%M%S}.json"
...
# ─────────────────── CLI ───────────────────
def build_parser():
    p = argparse.ArgumentParser(description="Evaluate model on Minerva‑Eval (VLLM).")
    p.add_argument("--model_path", required=True, help="HF model or local checkpoint")
    p.add_argument("--dataset_path", required=True, help="minerva_eval.json file")
