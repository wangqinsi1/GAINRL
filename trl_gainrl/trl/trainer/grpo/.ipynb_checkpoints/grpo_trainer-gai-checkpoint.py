...
class GRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # answers_output = [x["answer"] for x in inputs]
...
#================================================================================
        ori_prompt_length = []
        prompt_inputs1 = self.processing_class(
            prompts_text[0], return_tensors="pt", padding=False, add_special_tokens=False
        )
        ori_prompt_length.append(prompt_inputs1["input_ids"].size(1))
        prompt_inputs2 = self.processing_class(
            prompts_text[1], return_tensors="pt", padding=False, add_special_tokens=False
        )
        ori_prompt_length.append(prompt_inputs2["input_ids"].size(1))
        prompt_inputs3 = self.processing_class(
            prompts_text[2], return_tensors="pt", padding=False, add_special_tokens=False
        )
        ori_prompt_length.append(prompt_inputs3["input_ids"].size(1))
        prompt_inputs4 = self.processing_class(
            prompts_text[3], return_tensors="pt", padding=False, add_special_tokens=False
        )
        ori_prompt_length.append(prompt_inputs4["input_ids"].size(1))
#================================================================================
        ...
        # If gradient checkpointing is used, fall back to original implementation
            ...
            def get_per_token_logps(model, input_ids, attention_mask, num_logits_to_keep, mini_batch_size, ori_prompt_length):
                ...
                for i in range(0, input_ids.size(0), mini_batch_size):
                    ...
                    act_in_dict={}
                    # hooks = register_act_hooks(model, act_in_dict)
                    ...
                    #增加了angle信息
                    # remove_hooks(hooks)
                    # last_tensor = act_in_dict['model.layers.23.mlp.up_proj']
                    # last_tensor = last_tensor.squeeze(0).float()
                    # last_tensor_normalized = F.normalize(last_tensor, p=2, dim=1)
                    # cos = last_tensor_normalized @ last_tensor_normalized.T 
                    # cos = cos * torch.tril(torch.ones_like(cos), diagonal=1)
                    # if i<4:
                    #     length = ori_prompt_length[0]
                    # elif i>=4 and i<8:
                    #     length = ori_prompt_length[1]
                    # elif i>=8 and i<12:
                    #     length = ori_prompt_length[2]
                    # else:
                    #     length = ori_prompt_length[3]
                    # pro_data = cos[160:length]
                ...
                return torch.cat(per_token_logps, dim=0), Angle_metric

            num_logits_to_keep = completion_ids.size(1)  # we only need to compute the logits for the completion tokens
            
            per_token_logps, Angle_metric = get_per_token_logps(
                model=model,
                input_ids=prompt_completion_ids,
                attention_mask=attention_mask,
                num_logits_to_keep=num_logits_to_keep,
                mini_batch_size=mini_batch_size,
                ori_prompt_length=ori_prompt_length
            )
            print(Angle_metric)
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                output_reward_func = reward_func(prompts=prompts, completions=completions, Angle_metric = Angle_metric, **reward_kwargs)
