...
class GRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.args.use_vllm:
            ...
            completion_ids_fix = completion_ids_fix[process_slice]
            ...
            completion_ids = (completion_ids[:4] + ground_truth_output_list[:4]+completion_ids[4:8] + ground_truth_output_list[4:8] +completion_ids[8:12] + ground_truth_output_list[8:12] +completion_ids[12:16] + ground_truth_output_list[12:16])
            
            completion_ids = pad(completion_ids, padding_value=self.processing_class.pad_token_id)
            prompt_inputs_repeated = torch.repeat_interleave(prompt_inputs["input_ids"], self.num_generations*2, dim=0)
        ...
        # If gradient checkpointing is used, fall back to original implementation
            ...
            prompt_mask_repeated = prompt_inputs["attention_mask"].repeat_interleave(self.num_generations*2, dim=0)
...
#=======================================================================================================
        ...
        A = m.sum(dim=1)/completion_mask.sum(dim=1)
        print(f"{A}")
        device = A.device
        A_reshaped = A.view(4, 8)
        _, local_indices = A_reshaped[:, 4:].topk(2, dim=1, largest=False)
        col_indices = local_indices + 4 
        row_indices = torch.arange(4, device=device).view(-1, 1)
        flattened_indices = row_indices * 8 + col_indices
        flattened_indices = flattened_indices.flatten()
        mask = torch.ones(32, dtype=torch.bool, device=device)
        mask[flattened_indices] = False
        per_token_logps = per_token_logps[mask]
        ref_per_token_logps = ref_per_token_logps[mask]
...
#=======================================================================================================
        # A = per_token_logps* completion_mask
        # A_reshaped = A.view(4, 8, A.size(1))
        # device = A.device
        # mask_indices = []
        
        # for i in range(4):
        #     candidate_values = A_reshaped[i, 4:8, 0] 
        #     _, idx = torch.topk(candidate_values, k=2, largest=False)
        #     masked_js = idx + 4
        #     for j in masked_js:
        #         A_reshaped[i, j] = 0
        #         global_index = i * 8 + j.item()
...
#========================================================================================================
        ...
        n = per_token_logps* completion_mask
        print(f"per_token_logps{per_token_logps.size()}")
        print(f"per_token_logps{n.sum(dim=1)/completion_mask.sum(dim=1)}")
        ...
        mean_grouped_rewards = mean_grouped_rewards.repeat_interleave(self.num_generations+2, dim=0)
        std_grouped_rewards = std_grouped_rewards.repeat_interleave(self.num_generations+2, dim=0)
