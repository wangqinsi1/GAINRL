...
class GRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        print(f"per_token_kl{per_token_kl.size()}")
        print(f"per_token_kl{per_token_kl.sum(dim=1)}")
        ...
        # _, min_indices = torch.topk(std_grouped_rewards, 16, largest=False)
        # advantages[min_indices] = 0

        # _, max_indices = torch.topk(std_grouped_rewards, 8, largest=True)
        # advantages[max_indices] = 0
        
        # _, min_indices = torch.topk(mean_grouped_rewards, 8, largest=False)
        # advantages[min_indices] = 0

        # _, max_indices = torch.topk(mean_grouped_rewards, 8, largest=True)
        # advantages[max_indices] = 0
        
        # # 用1/-1替代
        # rewards = rewards.view(4, 2)
        # advantages = advantages.view(4, 2)
        # mask_all_zero = (rewards == 0).all(dim=1)
        # mask_all_three = (rewards == 3).all(dim=1)
        # advantages[mask_all_zero] = 1.5
        # advantages[mask_all_three] = 1.5
