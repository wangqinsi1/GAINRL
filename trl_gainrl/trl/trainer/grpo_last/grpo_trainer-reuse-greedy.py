...
#========================================================================================================================
        ...
        if hasattr(self, "good_logit_cache") and self.good_logit_cache is not None:
            ...
            loss_stored = torch.exp(stored_per_token_logps_current - stored_per_token_logps_current.detach()) * advantages_stored.unsqueeze(1)
            ...
            # final_loss = torch.cat((loss_current, loss_stored)).mean()
        ...
        if hasattr(self, "good_logit_cache") and self.good_logit_cache is not None:
            ...
            merged_seq =cache["seq"]+curr_seq
            merged_attn = cache["attn"]+curr_attn
            merged_token_logps = cat_cur_old(cache["stored_token_logps"], curr_token_logps)
            merged_ref_logps = cat_cur_old(cache["stored_ref_per_token_logps"], curr_ref_logps)
            merged_completion_mask = cat_cur_old(cache["completion_mask"], curr_completion_mask)
            merged_num_logits_to_keep =cache["num_logits"] + curr_num_logits_to_keep
            merged_advatages =torch.cat([cache["advatages"], advantages_current],dim=0)
            merged_std =torch.cat([cache["std"], std_grouped_rewards],dim=0)
            
        else:
            merged_seq = curr_seq
            merged_attn = curr_attn
            merged_token_logps = curr_token_logps
            merged_ref_logps = curr_ref_logps
            merged_completion_mask = curr_completion_mask
            merged_num_logits_to_keep = curr_num_logits_to_keep
            merged_advatages = advantages_current
            merged_std = std_grouped_rewards
            
    
        # 根据合并的 raw reward 重新归一化，计算合并优势
        # global_mean = merged_reward.mean()
        # global_std = merged_reward.std() + 1e-4
        # merged_advantage = (merged_reward - global_mean) / global_std 
        
        # # 选取优势最高的 topk 响应
        # if merged_advantage.size(0) >= topk:
        #     _, best_indices = torch.topk(merged_advantage, topk, largest=True)
        # else:
        #     best_indices = torch.arange(merged_advantage.size(0), device=device)
        ...
        if merged_advatages.size(0) >= topk:
            _, best_indices = torch.topk(torch.abs(merged_advatages), topk, largest=True)
        else:
            best_indices = torch.arange(merged_advatages.size(0), device=device)
        ...
        self.good_logit_cache = {
            ...
            "std": merged_std[best_indices].detach()*gamma_reward,
