...
#================================================================================
        ori_prompt_length = []
        num_prompts = min(len(prompts_text), 4)
        
        for i in range(num_prompts):
            
            prompt_inputs_collect = self.processing_class(
                prompts_text[i],
                return_tensors="pt",
                padding=False,
                add_special_tokens=False
            )
            ori_prompt_length.append(prompt_inputs_collect["input_ids"].size(1))
#================================================================================
        ...
        # If gradient checkpointing is used, fall back to original implementation
            ...
            def get_per_token_logps(model, input_ids, attention_mask, num_logits_to_keep, mini_batch_size, ori_prompt_length):
                ...
                for i in range(0, input_ids.size(0), mini_batch_size):
                    ...
                    # pro_data = cos[110:length]
                    # pro_data = pro_data.mean()
                    pro_data = cos
